...
def createSwapUsageAlarm(db, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createIopsAlarm(db, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createThroughputAlarm(db, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createDatabaseConnectionsAlarm(db, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def lambda_handler(event, context):
    ...
    for db in dbList:
        ...
        alarmName, created = createSwapUsageAlarm(db, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createIopsAlarm(db, instanceTypeMap, alarmNames)
        numOfAlarmsCreated += 1 if created else 0 
        alarmName, created = createThroughputAlarm(db, instanceTypeMap, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createInstanceNetworkBandwidthlarm(db, instanceTypeMap, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createDatabaseConnectionsAlarm(db, instanceTypeMap, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
