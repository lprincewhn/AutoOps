...
def createRedClusterAlarm(domain, alarmNames):
...
def createYellowClusterAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createClusterIndexWritesBlockedAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createFreeStorageSpaceAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createNodesAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createAutomatedSnapshotFailureAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createCPUUtilizationAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createCPUCreditBalanceAlarm(domain, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createJVMMemoryPressureAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createKMSKeyErrorAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createKMSKeyInaccessibleAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createIopsAlarm(domain, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createThroughputAlarm(domain, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createActiveShardsAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createMasterReachableFromNodeAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createThreadpoolWriteQueueAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createAvgThreadpoolSearchQueueAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createMaxThreadpoolSearchQueueAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createThreadpoolSearchRejectedAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createThreadpoolWriteRejectedAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def create5xxRateAlarm(domain, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def lambda_handler(event, context):
    ...
    for domain in domainList:
        ...
        alarmName, created = createRedClusterAlarm(domainStatus, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createYellowClusterAlarm(domainStatus, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createClusterIndexWritesBlockedAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createFreeStorageSpaceAlarm(domainStatus, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createNodesAlarm(domainStatus, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createAutomatedSnapshotFailureAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createCPUUtilizationAlarm(domainStatus, alarmNames)
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createJVMMemoryPressureAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createIopsAlarm(domainStatus, instanceTypeMap, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createThroughputAlarm(domainStatus, instanceTypeMap, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createActiveShardsAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createMasterReachableFromNodeAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createThreadpoolWriteQueueAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createAvgThreadpoolSearchQueueAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createMaxThreadpoolSearchQueueAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createThreadpoolSearchRejectedAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        alarmName, created = createThreadpoolWriteRejectedAlarm(domainStatus, alarmNames)   
        numOfAlarmsCreated += 1 if created else 0
        ...
        if domainStatus["EncryptionAtRestOptions"]["Enabled"]:
            ...
            numOfAlarmsCreated += 1 if created else 0
            alarmName, created = createKMSKeyInaccessibleAlarm(domainStatus, alarmNames)
            numOfAlarmsCreated += 1 if created else 0
