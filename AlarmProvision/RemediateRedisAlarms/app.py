...
def populateAlarmDef(alarmObject, tags, metricName):
    ...
    if not alarmdef:
        ...
        alarmdef_file = 'redis_alarms.yaml'
    ...
    matches = list(filter(lambda x:alarmObject.get(x['Key']) and alarmObject[x['Key']] in x['Value'], alarmdef['RedisAlarm']['Filters']))
    logger.info(f'Found {len(matches)} filters matched for metric {metricName}')
    alarmObject[metricName] = matches[0][metricName] if matches and matches[0].get(metricName)  else alarmdef['RedisAlarm']['Default'][metricName]
    logger.info(f'{alarmObject["AlarmDef"]}')
    
def createCPUUtilizationAlarm(region, node, tags, alarmExisting, alarmsCreated):
    populateAlarmDef(node, tags, 'CPUUtilization')
    if not node['AlarmDef']['Enabled']:
        return
    nodeId = node["CacheClusterId"]
    alarmName = f'AWS/ElastiCache-CPUUtilization-{nodeId}'
    if alarmName in alarmExisting and node['AlarmDef']['Enabled']:
        # 从现存告警列表中移除表示保留该告警
        logger.info(f'{alarmName} existed. skip it...')
        alarmExisting.remove(alarmName)
        return
    threshold = node['AlarmDef']['Threshold']
    logger.info(f'Threshold of {alarmName}: {threshold}')
    alarm_actions, ok_actions = common.getSSMActions(account_id, region, node['AlarmDef'])
...
def createEngineCPUUtilizationAlarm(region, node, tags, alarmExisting, alarmsCreated):
    populateAlarmDef(node, tags, 'EngineCPUUtilization')
    if not node['AlarmDef']['Enabled']:
        return
    nodeId = node["CacheClusterId"]
...
def createCPUCreditBalanceAlarm(region, node, instanceTypes, tags, alarmExisting, alarmsCreated):
    populateAlarmDef(node, tags, 'CPUCreditBalance')
    if not node['AlarmDef']['Enabled']:
        return
    nodeId = node["CacheClusterId"]
...
def createDatabaseMemoryUsagePercentageAlarm(region, node, tags, alarmExisting, alarmsCreated):
    populateAlarmDef(node, tags, 'DatabaseMemoryUsagePercentage')
    if not node['AlarmDef']['Enabled']:
        return
    nodeId = node["CacheClusterId"]
...
def createSwapUsageAlarm(region, node, tags, alarmExisting, alarmsCreated):
    populateAlarmDef(node, tags, 'SwapUsageMB')
    if not node['AlarmDef']['Enabled']:
        return
    nodeId = node["CacheClusterId"]
    ...
    threshold = node['AlarmDef']['Threshold']*1024*1024
    logger.info(f'Threshold of {alarmName}: {threshold}')
    alarm_actions, ok_actions = common.getSSMActions(account_id, region, node['AlarmDef'])
    logger.info(f'AlarmActions of {alarmName}: {alarm_actions}')
    logger.info(f'OkActions of {alarmName}: {ok_actions}')
...
def createInstanceNetworkBandwidthlarm(region, node, instanceTypes, tags, alarmExisting, alarmsCreated):
    populateAlarmDef(node, tags, 'MaxNetworkBandwidth')
    populateAlarmDef(node, tags, 'BaseNetworkBandwidth')
    if not node['AlarmDef']['Enabled']:
        return
    nodeId = node["CacheClusterId"]
