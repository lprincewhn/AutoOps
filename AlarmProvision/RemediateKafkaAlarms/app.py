...
def createKafkaDataLogsDiskUsedAlarm(node, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createHeapMemoryAfterGCAlarm(node, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createIopsAlarm(node, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def createThroughputAlarm(node, instanceTypes, alarmNames):
    sns_topic = os.getenv('SNSTopicArn')
    actions_enable = (sns_topic!=None) 
    actions = [sns_topic] if sns_topic else []
    client = boto3.client('cloudwatch')
...
def lambda_handler(event, context):
    ...
    for cluster in kafkaClusterList:
        ...
        for node in nodeList:
            ...
            alarmName, created = createKafkaDataLogsDiskUsedAlarm(node, instanceTypeMap, alarmNames)  
            numOfAlarmsCreated += 1 if created else 0
            alarmName, created = createHeapMemoryAfterGCAlarm(node, instanceTypeMap, alarmNames) 
            numOfAlarmsCreated += 1 if created else 0
            ...
            if cluster["EnhancedMonitoring"] in ["PER_BROKER", "PER_TOPIC_PER_BROKER", "PER_TOPIC_PER_PARTITION"]:
                alarmName, created = createIopsAlarm(node, instanceTypeMap, alarmNames)   
                numOfAlarmsCreated += 1 if created else 0
                alarmName, created = createThroughputAlarm(node, instanceTypeMap, alarmNames)   
                numOfAlarmsCreated += 1 if created else 0
