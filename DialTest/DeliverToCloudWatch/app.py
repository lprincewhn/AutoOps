...
dims = '[' + os.getenv('CLOUDWATCH_DIMENSIONS', '[],["City"]') + ']' 
...
def parseTimingHeader(timings, metrics):
    for timing in timings:
        logger.debug(f'Item: {timing}')
        record = {}
        ...
        if int(record['cdn-upstream-dns'])>0:
        ...
        if int(record['cdn-upstream-connect'])>0:
        ...
        if int(record['cdn-upstream-fbl'])>0:
        ...
        if int(record['cdn-upstream-fbl'])<metrics['cdn-upstream-fbl-min'].get(metric_key, 0):
...
def lambda_handler(event, context):
    ...
    for popid in pop_list.split(','):
        ...
        for i in range(2):
            try:
                r = requests.head(url, headers={'Host': cf_domainname}, timeout=10)
    ...
    parseTimingHeader(timings, metrics)
    ...
    for k,v in metrics['cdn-upstream-fbl-sum'].items():
        emfdata = {
            ...
            'cdn-upstream-dns': metrics['cdn-upstream-dns-sum'].get(k, 0)/metrics['cdn-upstream-dns-cnt'].get(k, 999999999),
            'cdn-upstream-connect': metrics['cdn-upstream-connect-sum'].get(k, 0)/metrics['cdn-upstream-connect-cnt'].get(k, 999999999),
            'cdn-upstream-fbl': metrics['cdn-upstream-fbl-sum'].get(k, 0)/metrics['cdn-upstream-fbl-cnt'].get(k, 0)
        }
        if use_emf:
